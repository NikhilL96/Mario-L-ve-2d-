...
function GeneratePoles(atlas, poleWidth, poleHeight)
    ...
    local spritesheet = {}
        ...
        for x = 0, 5 do
            spritesheet[sheetCounter] =
                love.graphics.newQuad(x * poleWidth, 0, poleWidth,
                poleHeight, atlas:getDimensions())
            sheetCounter = sheetCounter + 1
        end
...
function GenerateFlags(atlas, flagWidth, flagHeight)
    ...
    for y =0, 2 do
        for x = 6, 8 do
            spritesheet[sheetCounter] =
                love.graphics.newQuad(x * flagWidth, y * flagHeight, flagWidth,
                flagHeight, atlas:getDimensions())
            sheetCounter = sheetCounter + 1
        end
    end
    return spritesheet
